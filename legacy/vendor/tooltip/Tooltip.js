...
(function (root, factory) {
	...
	function _createTooltip(text) {
		...
		tooltipElm.className = 'ren-tooltip';
		
		var tooltipArrow = document.createElement("div");
		tooltipArrow.className = 'ren-tooltip-arrow';
		tooltipElm.appendChild(tooltipArrow);

		var tooltipRest = document.createElement("div");
		tooltipRest.className = 'ren-tooltip-body';
		var tooltipText = document.createTextNode(text);
		tooltipRest.appendChild(tooltipText);
		tooltipElm.appendChild(tooltipRest);
	...
	function _onElementMouseMove(evt) {
		...
		var elmBoundingRect = evt.target.getBoundingClientRect();
		if (tooltipElm) {
			tooltipTop = elmBoundingRect.bottom;
			tooltipLeft = elmBoundingRect.left + elmBoundingRect.width /2 - tooltipElm.offsetWidth/2 ;
